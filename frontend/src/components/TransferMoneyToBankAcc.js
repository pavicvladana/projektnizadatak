...
export default function TransferMoneyToUser() {
    ...
    const onSubmit = data => {
        ...
        const requestOptions = {
          ...
          fetch("http://localhost:5000/user/sent-to-bank-account", requestOptions)
            .then((res) => res.json())
            .then((data) => {
                console.log(data)
              if (data.error) {
    ...
    return (
        <>
         <h1>Transfer money to bank account.</h1>
         <br></br>         
         ...
         {account ?
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <input type="text" placeholder="To" {...register("to", {required: true, pattern: /[0-9]{3}[-][0-9]{7}[-][0-9]{3}/i})} />
